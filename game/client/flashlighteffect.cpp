...
static ConVar r_flashlight_delay_factor( "r_flashlight_delay_factor", "0.9", FCVAR_CHEAT );
#endif
...
void CFlashlightEffect::UpdateLight(const Vector &vecPos, const Vector &vecDir, const Vector &vecRight, const Vector &vecUp, int nDistance)
{
	...
	float factor = r_flashlight_delay_factor.GetFloat();
	lightPos = lightPos * factor + vecPos * (1 - factor);
	lightDir = lightDir * factor + vecDir * (1 - factor);
	lightRight = lightRight * factor + vecRight * (1 - factor);
	lightUp = lightUp * factor + vecUp * (1 - factor);
	#endif

	if( r_newflashlight.GetBool() )
	{
		#ifdef FLASHLIGHT_DELAY
		UpdateLightNew( lightPos, lightDir, lightRight, lightUp );
		#else
	...
	{
		#ifdef FLASHLIGHT_DELAY
        UpdateLightOld( lightPos, lightDir, nDistance );
		#else
		UpdateLightOld( vecPos, vecDir, vecRight, vecUp );
		#endif
	}
}
