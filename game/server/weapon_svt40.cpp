...
#ifdef CLIENT_DLL
#define CWeaponSVT40 C_WeaponAK47
#endif
...
class CWeaponSVT40 : public CBaseHL2MPCombatWeapon
{
  public:
 
    DECLARE_CLASS( CWeaponSVT40, CBaseHL2MPCombatWeapon );
    CWeaponSVT40(void);
  ...
  private:
    CWeaponSVT40( const CWeaponSVT40 & );
};
 
IMPLEMENT_NETWORKCLASS_ALIASED( WeaponAK47, DT_WeaponAK47 )
 
BEGIN_NETWORK_TABLE( CWeaponSVT40, DT_WeaponAK47 )
#ifdef CLIENT_DLL
...
BEGIN_PREDICTION_DATA( CWeaponSVT40 )
END_PREDICTION_DATA()
 
LINK_ENTITY_TO_CLASS( weapon_AK47, CWeaponSVT40 );
...
//-----------------------------------------------------------------------------
void CWeaponSVT40::DryFire( void )
{
  WeaponSound( EMPTY );
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponSVT40::ItemPreFrame( void )
{
  GetStance();
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponSVT40::ItemBusyFrame( void )
{
...
//----------------------------------------------------------------------------- 
bool CWeaponSVT40::Holster(CBaseCombatWeapon *pSwitchingTo /* = NULL */)
{
  if ( m_bInZoom )
    { ToggleZoom(); }
...
//-----------------------------------------------------------------------------
void CWeaponSVT40::ToggleZoom( void )
{
  CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
...
//-----------------------------------------------------------------------------
void CWeaponSVT40::CheckZoomToggle( void )
{
  CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
...
//----------------------------------------------------------------------------- 
void CWeaponSVT40::GetStance( void )
{
  CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
  if ( pPlayer == NULL )
    { return; }
